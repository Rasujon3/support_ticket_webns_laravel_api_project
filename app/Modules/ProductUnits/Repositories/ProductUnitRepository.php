...
namespace App\Modules\ProductUnits\Repositories;
...
class ProductUnitRepository
{
    ...
    public function getSummaryData()
    {
        ...
        $totalProductUnit = ProductUnit::get()->count();

        return [
            'totalProductUnit' => $totalProductUnit,
        ];
    }
    public function all()
    {
        return ProductUnit::cursor(); // Load all records
    }
    ...
    public function store(array $data): ?ProductUnit
    {
        try {
            ...
            // Create the ProductUnit record in the database
            $store = ProductUnit::create($data);
...
//            ]);
            ...
            // Log the error
            Log::error('Error in storing ProductUnit: ' . $e->getMessage(), [
...
//            ]);
    ...
    {
        $store = ProductUnit::where('id', $id)->first();
        return $store;
    }
